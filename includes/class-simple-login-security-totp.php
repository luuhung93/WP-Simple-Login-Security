...
final class Simple_Login_Security_TOTP {
    public function verify_for_login($user_id, $secret, $otp, $window) {
        ...
        $last_counter = (int) get_user_meta($user_id, Simple_Login_Security_Plugin::META_LAST_COUNTER, true);
        ...
        for ($offset = -$window; $offset <= $window; $offset++) {
            ...
            if (hash_equals($expected, $otp)) {
                update_user_meta($user_id, Simple_Login_Security_Plugin::META_LAST_COUNTER, $slot);
                return true;
            }
        }
