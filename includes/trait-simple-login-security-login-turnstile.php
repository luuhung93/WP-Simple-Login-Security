...
trait Simple_Login_Security_Login_Turnstile_Trait {
    public function render_login_field() {
        ...
        if ($turnstile_site_key !== '' && $this->is_turnstile_enabled()) :
            ?>
        <div class="simple-login-security-turnstile">
    ...
    public function enqueue_login_assets() {
        ...
        wp_enqueue_script(
            'simple-login-security-turnstile',
    ...
    public function validate_turnstile_login($user, $username, $password) {
        ...
        if ($token === '') {
            return new WP_Error('simple_login_security_turnstile_missing', __('Please complete the security check.', 'simple-login-security'));
        }

        if (!$this->verify_turnstile_token($token)) {
            return new WP_Error('simple_login_security_turnstile_invalid', __('Security check failed. Please try again.', 'simple-login-security'));
        }
    ...
    public function handle_login_otp_challenge() {
        ...
        if (!is_array($challenge) || empty($challenge['user_id'])) {
            $this->render_login_otp_screen(
                new WP_Error('simple_login_security_invalid_challenge', __('Your verification session is invalid or expired. Please login again.', 'simple-login-security')),
                $token
            );
            exit;
        }
        ...
        if (!($user instanceof WP_User)) {
            ...
            $this->render_login_otp_screen(
                new WP_Error('simple_login_security_invalid_user', __('User not found. Please login again.', 'simple-login-security')),
                $token
            );
            exit;
        }
        ...
        if (!$this->is_enabled_for_user($user->ID)) {
            $this->delete_login_otp_challenge($token);
            $this->render_login_otp_screen(
                new WP_Error('simple_login_security_disabled', __('Two-factor authentication is disabled for this account. Please login again.', 'simple-login-security')),
                $token
            );
            exit;
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($otp === '') {
                $this->render_login_otp_screen(
                    new WP_Error('simple_login_security_missing_otp', __('Authentication code is required.', 'simple-login-security')),
                    $token
                );
                exit;
            }
    ...
    private function validate_user_otp_input(WP_User $user, $otp) {
        $secret = (string) get_user_meta($user->ID, Simple_Login_Security_Plugin::META_SECRET, true);
        if ($secret === '') {
            return new WP_Error('simple_login_security_missing_secret', __('Two-factor is enabled but not configured for this account.', 'simple-login-security'));
        }
        ...
        return new WP_Error('simple_login_security_invalid_otp', __('Invalid authentication or backup code.', 'simple-login-security'));
    }
    ...
    private function get_turnstile_site_key() {
        return (string) get_option(Simple_Login_Security_Plugin::OPTION_TURNSTILE_SITE_KEY, '');
    }

    private function get_turnstile_secret_key() {
        return (string) get_option(Simple_Login_Security_Plugin::OPTION_TURNSTILE_SECRET_KEY, '');
    }

    private function is_turnstile_enabled() {
        if (get_option(Simple_Login_Security_Plugin::OPTION_TURNSTILE_ENABLED, '0') !== '1') {
            return false;
        }
    ...
    private function get_submitted_otp($field = 'simple_login_security_otp') {
    ...
    private function verify_totp($user_id, $secret, $otp) {
        $window = (int) apply_filters('simple_login_security_window', 1, $user_id);
        if ($window < 0) {
            $window = 0;
        }
    ...
    private function start_login_otp_challenge(WP_User $user) {
        ...
        set_transient(
            ...
            $challenge,
            Simple_Login_Security_Plugin::LOGIN_OTP_TTL
        );
    ...
    private function render_login_otp_screen($error, $token) {
        ...
        login_header(__('Two-Factor Authentication', 'simple-login-security'), '', $errors);
        ?>
        <form method="post" action="<?php echo esc_url($this->get_login_otp_url($token)); ?>">
            <p>
                <label for="simple_login_security_otp"><?php esc_html_e('Authentication code', 'simple-login-security'); ?><br />
                    <input type="text" name="simple_login_security_otp" id="simple_login_security_otp" class="input" value="" size="20" inputmode="numeric" autocomplete="one-time-code" />
                </label>
            </p>
            <p class="submit">
                <input type="submit" class="button button-primary button-large" value="<?php esc_attr_e('Verify', 'simple-login-security'); ?>" />
            </p>
            <p class="forgetmenot">
                <a href="<?php echo esc_url(wp_login_url()); ?>"><?php esc_html_e('Back to login', 'simple-login-security'); ?></a>
            </p>
        </form>
        <?php
        login_footer();
    ...
    private function get_login_otp_url($token) {
        $url = add_query_arg('action', 'simple_login_security_otp', wp_login_url());
        if ($token === '') {
            return $url;
        }
        return add_query_arg('simple_login_security_token', rawurlencode($token), $url);
    }

    private function get_login_otp_token() {
        if (isset($_REQUEST['simple_login_security_token'])) {
            $token = sanitize_text_field(wp_unslash($_REQUEST['simple_login_security_token']));
    ...
    private function get_login_otp_transient_key($token) {
        return Simple_Login_Security_Plugin::LOGIN_OTP_TRANSIENT_PREFIX . $token;
    }
